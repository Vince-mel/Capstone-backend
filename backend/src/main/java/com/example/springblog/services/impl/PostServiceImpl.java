...
//        post.setImageName("default.jpg");
    ...
    @Override
    public List<PostDto> getPostByTitles(String keywords) {
        String[] words = keywords.split("\\s+"); // Divide la stringa in parole basate su spazi.
        List<Post> postList = new ArrayList<>();

        for (String word : words) {
            List<Post> postsForWord = postRepository.findByTitleContainingIgnoreCase(word);
            postList.addAll(postsForWord);
        }

        // Elimina i duplicati (opzionale, a seconda dei requisiti)
        List<Post> distinctPosts = postList.stream().distinct().collect(Collectors.toList());

        return distinctPosts.stream().map(post -> modelMapper.map(post, PostDto.class)).collect(Collectors.toList());
    }
